...
#region Edit by Miryn
    ...
    public string str
    {
        get
        {
            ...
            throw new Exception("Can't get str because str is null and type is not number.");
        }
        set
        {
            _str = value;
...
#region Add by Miryn
    ...
    public static string GetNormalStringWithDoubleQuote(string key)
    {
        return "\"" + key + "\"";
    }
    ...
    public char GetChar()
    {
        if (type == Type.STRING)
            return str[0];
        throw new Exception("Cannot convert to char becasue type is not string.");
    }
    ...
    public bool GetBool()
    {
        if (type == Type.BOOL)
            return b;
        throw new Exception("Type is not boolean.");
    }

    public string GetString(bool unEscapeString = false)
    {
        if (type == Type.STRING)
        {
            if (unEscapeString)
                return str.UnEscapeString();
            else
                return str;
        }
    ...
    /// Convert to integer.
    /// </summary>
    /// <value>The interger convert from str.</value>
    public int GetAsInteger()
    {
        return TryCatchForGetAs(() => int.Parse(str));
    }
    ...
    /// </summary>
    /// <returns></returns>
    public UInt32 GetAsUnsignedInteger()
    {
        return TryCatchForGetAs(() => UInt32.Parse(str));
    }

    /// <summary>
    /// Convert to float.
    /// </summary>
    /// <value>The float convert from str.</value>
    public float GetAsFloat()
    {
        return TryCatchForGetAs(() => float.Parse(str));
    }

    /// <summary>
    /// Convert to double.
    /// </summary>
    /// <value>The double convert from str.</value>
    public double GetAsDouble()
    {
        return TryCatchForGetAs(() => double.Parse(str));
    }

    /// <summary>
    /// Convert to char.
    /// </summary>
    /// <value>The char convert from str.</value>
    ...
    /// </summary>
    /// <returns>A <see cref="System.String"/> that represents the current <see cref="JSONObject"/>.</returns>
    public string GetAsString(bool unEscapeString = false)
    {
        try
        {
            if (unEscapeString)
    ...
    /// Convert to boolean.
    /// </summary>
    /// <value><c>true</c> if str is "1"; otherwise, <c>false</c>.</value>
    public bool GetAsBool()
    {
        try
        {
            int num = GetAsInteger();
            return num == 1;
        }
    ...
    {
        return TryCatchForGetAs(() =>
        {
            var jsonObjectList = new List<JSONObject>(list);

            jsonObjectList.ForEach(jsonObject =>
                jsonObject.tempKey = tempKey);
    ...
    private T TryCatchForGetAs<T>(Func<T> func)
    {
        try
        {
            return func();
        ...
        {
            throw new FormatException("Input string was not in the correct format with " + GetNormalStringWithDoubleQuote(tempKey) + " key.");
        }
...
#region PARSE
    ...
    {   //create a new JSONObject from a string (this will also create any children, and parse the whole string)
        Debug.Log("JSONObject try contructor :: " + str);
                ...
                else
                {
                    ...
                    switch (str[offset])
                    {
                        ...
                        default:
                            ...
                            {
                                type = Type.NULL;
                                Debug.LogWarning("improper JSON formatting: " + str + " exception:" + e.Message);
                            }

                            Debug.Log("End Parse Finish :: " + type);
                            return;
                    }
                    string propName = "";
    ...
    public void SetAsJsonArrayList(List<JSONObject> objectList)
    {
        list = objectList;
        type = JSONObject.Type.ARRAY;
    }
...
#region STRINGIFY
    ...
    public JSONObject this [string index]
    {
        get
        {
            ...
            throw new KeyNotFoundException("Not found \"" + index + "\" key.");
        }
        set
        {
            if (HasField(index))
