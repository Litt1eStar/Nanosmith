...
public abstract class TbaTweener : MonoBehaviour
{
	/// <summary>
	/// Current tween that triggered the callback function.
	/// </summary>

	static public TbaTweener current;
	...
	static public T Begin<T> (GameObject go, float duration, float delay = 0f) where T : TbaTweener
	{
		...
		if (comp == null)
		{
			...
			if (comp == null)
			{
				Debug.LogError("Unable to add " + typeof(T));
				return null;
			}
		}
#endif
