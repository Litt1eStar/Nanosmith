...
static public class TbaTweeningMath
{
	/// <summary>
 ...
 //				if (v.z < vMin.z) vMin.z = v.z;
	...
	static public float DistanceToRectangle (Vector2[] screenPoints, Vector2 mousePos)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			Vector3 v0 = screenPoints[TbaTweeningMath.RepeatIndex(i, 4)];
			Vector3 v1 = screenPoints[TbaTweeningMath.RepeatIndex(j, 4)];
		...
		if (!oddNodes)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				Vector3 v1 = screenPoints[TbaTweeningMath.RepeatIndex(i + 1, 4)];
...
//	/// </summary>
    ...
    static public void MoveRect (RectTransform rect, float x, float y)
	{
        ...
        rect.anchoredPosition += new Vector2(ix, iy);
	}
